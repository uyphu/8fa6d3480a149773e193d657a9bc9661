...
app
   .controller('PlaylistController', function ($scope, $rootScope, $timeout, usSpinnerService, 
		   Tube, TubeSearch, Playlist, ParseLinks, localStorageService) {
       $scope.tubes = [];
       $scope.page = 1;
       $scope.cursor = null;
       ...
       function initData(cursor, count) {
    	   $scope.startSpin();
    	   Playlist.search($scope.querySearch, cursor, count).then(function(data) {
    		   $scope.stopSpin();
    		   if (data != null) {
    			   if (data.items != null) {
